...
def calcular_metricas(procesos):
    ...
    promedio_retorno = retorno_total / n if n > 0 else 0
    promedio_espera = espera_total / n if n > 0 else 0
    promedio_respuesta = respuesta_total / n if n > 0 else 0

    tiempo_total = max(p['fin'] for p in procesos) - min(p['llegada'] for p in procesos) if n > 0 else 0
    throughput = n / tiempo_total if tiempo_total > 0 else 0

    min_retorno = min(retornos) if retornos else 0
    max_retorno = max(retornos) if retornos else 0

    eps = 1e-9
    servicios = [1.0 / (r + eps) for r in retornos] if retornos else [0]
    suma_s = sum(servicios)
    suma_s_cuad = sum(s * s for s in servicios)
    if n > 0 and suma_s_cuad > 0:
        fairness = (suma_s * suma_s) / (n * suma_s_cuad)
    else:
        fairness = 0.0
