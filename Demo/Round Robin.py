...
def crear_procesos_personalizado():
    ...
    input_llegada = ""
    input_duracion = ""
    input_quantum = "3"  # Quantum por defecto
    scroll_offset = 0
    max_scroll = 0
    
    # Centrar el panel principal
    panel_width = 700
    panel_x = (1700 - panel_width) // 2
    
    # Botones centrados
    boton_agregar = pygame.Rect(panel_x + 50, 720, 200, 50)
    boton_finalizar = pygame.Rect(panel_x + 270, 720, 200, 50)
    boton_regresar = pygame.Rect(panel_x + 490, 720, 160, 50)
    
    # Campos de input centrados
    input_llegada_rect = pygame.Rect(panel_x + 250, 100, 150, 40)
    input_duracion_rect = pygame.Rect(panel_x + 250, 160, 150, 40)
    input_quantum_rect = pygame.Rect(panel_x + 250, 220, 150, 40)
    
    # Área de scroll para la lista de procesos
    lista_rect = pygame.Rect(panel_x + 50, 290, 600, 400)
    scroll_bar_rect = pygame.Rect(panel_x + 660, 290, 20, 400)
    ...
    while corriendo:
        ...
        titulo_rect = titulo.get_rect(center=(850, 40))
        ventana.blit(titulo, titulo_rect)
        
        # Labels
        label_llegada = fuente.render("Tiempo de llegada:", True, color_texto)
        ventana.blit(label_llegada, (panel_x + 50, 110))
        
        label_duracion = fuente.render("Duración:", True, color_texto)
        ventana.blit(label_duracion, (panel_x + 50, 170))
        
        label_quantum = fuente.render("Quantum:", True, color_texto)
        ventana.blit(label_quantum, (panel_x + 50, 230))
        ...
        color_quantum = color_input_activo if input_activo == "quantum" else color_input
        pygame.draw.rect(ventana, color_quantum, input_quantum_rect, border_radius=5)
        pygame.draw.rect(ventana, color_texto, input_quantum_rect, 2, border_radius=5)
        texto_quantum = fuente_input.render(input_quantum, True, color_texto)
        ventana.blit(texto_quantum, (input_quantum_rect.x + 10, input_quantum_rect.y + 10))
        ...
        ventana.blit(label_lista, (lista_rect.x + 10, lista_rect.y + 5))
        ...
        color_btn_reg = (239, 68, 68) if not boton_regresar.collidepoint(mouse_pos) else (220, 38, 38)
        pygame.draw.rect(ventana, color_btn_reg, boton_regresar, border_radius=10)
        texto_reg = fuente_titulo.render("Regresar", True, (255, 255, 255))
        ventana.blit(texto_reg, (boton_regresar.x + 30, boton_regresar.y + 12))
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.button == 1:
                    ...
                    elif input_quantum_rect.collidepoint(event.pos):
                        input_activo = "quantum"
                    ...
                    if boton_agregar.collidepoint(event.pos):
                        ...
                        try:
                            quantum_value = int(input_quantum) if input_quantum else 3
                            if quantum_value > 0:
                                if len(procesos) > 0:
                                    corriendo = False
                                else:
                ...
                elif input_activo == "quantum":
                    if event.key == pygame.K_BACKSPACE:
                        input_quantum = input_quantum[:-1]
                        ...
                        input_quantum += event.unicode
    
    try:
        quantum_final = int(input_quantum) if input_quantum else 3
    except ValueError:
        quantum_final = 3
    
    return procesos, quantum_final
...
def reset_simulacion(quantum=3, escenario="A"):
    if escenario == "PERSONALIZADO":
        resultado = crear_procesos_personalizado()
...
def simular_round_robin_visual():
    ...
    while corriendo:
        ...
        procesos_visibles = min(len(en_cola), 7)
